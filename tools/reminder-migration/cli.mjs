...
import { migrateReminderArchiveFile, ReminderMigrationError } from "./index.js";

const usage = `Usage:
  node tools/reminder-migration/cli.mjs --input <archive-v2.json> --output <archive-v3.json> [--overwrite]
`;
...
try {
  ...
  if (options.help) {
    ...
    await migrateReminderArchiveFile(options);
    process.stdout.write(`Reminder-safe archive created: ${options.outputPath}\n`);
  }
} catch (error) {
